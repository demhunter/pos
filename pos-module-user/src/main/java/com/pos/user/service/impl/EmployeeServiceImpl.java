...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public List<SessionUserDto> findEmployeeForImMember(String name, String userPhone, String company, Boolean
            deleted, Boolean available) {
        return employeeDao.findEmployeeForImMember(name,userPhone,company,deleted,available);
    }

    @Override
    public List<EmployeeDto> findByCompaniesId(
            List<Long> companiesId, int maxSize, boolean disable, boolean deleted) {
        FieldChecker.checkEmpty(companiesId, "companiesId");
        FieldChecker.checkMinMaxValue(maxSize, 1, 50, "maxSize");
        return employeeDao.findEmployeesByCompaniesId(companiesId.toArray(
                new Long[companiesId.size()]), maxSize, deleted ? null : false, disable ? null : true);
    }
    ...
    @Override
    public List<EmployeeInfoDto> findByUserDetailType(Byte userType, Long companyId, boolean quitJob, boolean disable, boolean deleted) {
        FieldChecker.checkEmpty(userType, "userType");
        FieldChecker.checkEmpty(companyId, "companyId");

        return employeeDao.findByUserDetailType(userType, companyId,  quitJob ? null : false,  disable ? null : true,  deleted ? null : false);
    }
}

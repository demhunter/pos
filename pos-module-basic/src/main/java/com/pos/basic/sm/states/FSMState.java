    ...
    private void updateNewTransitionMap() {
        if (_transitionMap!=null) {
            ...
            while(iter.hasNext()) {
                ...
                String[] val = ((String) next.getValue()).split(":");
                this._transitions.put(next.getKey(), new FSMTransitionInfo((String) next.getKey(), val[0]));
                FSMAction action;
                try {
                    WebApplicationContext applicationContext = ContextLoader.getCurrentWebApplicationContext();
                    action = (FSMAction)applicationContext.getBean(val[1]);
                } catch (Exception e) {
                    throw new IllegalParamException("状态机配置有误！", e);
                }
                this.addMessageAction((String) next.getKey(), action);
            }
        }
    }
